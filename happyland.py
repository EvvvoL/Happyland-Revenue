...
# 模型验证函数 - 新增功能
def validate_model_on_new_data(models, feature_names, fy1_df, fy0_tail=None):
    """在FY1数据上验证模型预测人数的准确性"""
    ...
    for target_name in ['Attendance_AP', 'Attendance_PAID']:
        model = models[target_name]
        
        # 选择特征矩阵
        if target_name == 'Attendance_AP':
            features = ap_feature_matrix
        else:
            features = paid_feature_matrix
        ...
        # 获取实际值
        actual_values = fy1_df[target_name].values if target_name in fy1_df.columns else None
    ...
    return validation_results, processed_fy1
    ...
    try:
        ...
        **FY1预测说明:**
        ...
        if FY1_FILE_PATH is not None:
            ...
            # ==================== 新增：模型验证步骤 ====================
            st.markdown("---")
            st.subheader("🧪 模型可靠性验证")
            
            with st.spinner('正在验证模型在新数据上的预测准确性...'):
                validation_results, processed_fy1 = validate_model_on_new_data(
                    models, feature_names, fy1_df, fy0_tail
                )
            ...
            with col1:
                st.subheader("🎫 年卡会员验证结果")
                if 'Attendance_AP' in validation_results and validation_results['Attendance_AP']['actual'] is not None:
                    ap_result = validation_results['Attendance_AP']
                    st.metric("MAPE (平均绝对百分比误差)", f"{ap_result['mape']:.2%}")
                    st.metric("MAE (平均绝对误差)", f"{ap_result['mae']:.2f}")
                    st.metric("预测准确率", f"{ap_result['accuracy']:.2f}%")
                    
                    # 设置验证标准
                    if ap_result['mape'] > 0.1:  # MAPE > 10%
                        st.error("❌ 年卡会员预测精度未达标")
                        validation_passed = False
                    elif ap_result['mape'] > 0.05:  # MAPE 5-10%
                        st.warning("⚠️ 年卡会员预测精度一般")
                    else:
                        st.success("✅ 年卡会员预测精度优秀")
                else:
                    st.warning("无法验证年卡会员预测：实际值不可用")
                    validation_passed = False
            
            with col2:
                st.subheader("✈️ 普通游客验证结果")
                if 'Attendance_PAID' in validation_results and validation_results['Attendance_PAID']['actual'] is not None:
                    paid_result = validation_results['Attendance_PAID']
                    st.metric("MAPE (平均绝对百分比误差)", f"{paid_result['mape']:.2%}")
                    st.metric("MAE (平均绝对误差)", f"{paid_result['mae']:.2f}")
                    st.metric("预测准确率", f"{paid_result['accuracy']:.2f}%")
                    
                    # 设置验证标准
                    if paid_result['mape'] > 0.1:  # MAPE > 10%
                        st.error("❌ 普通游客预测精度未达标")
                        validation_passed = False
                    elif paid_result['mape'] > 0.05:  # MAPE 5-10%
                        st.warning("⚠️ 普通游客预测精度一般")
                    else:
                        st.success("✅ 普通游客预测精度优秀")
                else:
                    st.warning("无法验证普通游客预测：实际值不可用")
                    validation_passed = False
            ...
            if 'Attendance_PAID' in validation_results and validation_results['Attendance_PAID']['actual'] is not None:
                ...
                fig_validation.add_trace(go.Scatter(
                    x=fy1_df['Date'],
                    y=paid_result['actual'],
                    name='普通游客-实际',
                    line=dict(color='red')
                ))
                fig_validation.add_trace(go.Scatter(
                    x=fy1_df['Date'],
                    y=paid_result['predictions'],
                    name='普通游客-预测',
                    line=dict(color='pink', dash='dash')
                ))
            
            fig_validation.update_layout(
                title="模型验证：预测人数 vs 实际人数",
                xaxis_title="日期",
                yaxis_title="人数",
                hovermode='x unified',
                height=400
            ...
            if validation_passed:
                st.success("🎉 模型验证通过！预测精度符合要求，可以进行收入预测。")
                
                # 只有验证通过才进行收入预测
                proceed_prediction = st.button("🚀 开始收入预测", type="primary")
            else:
                st.error("❌ 模型验证未通过！预测精度不达标，建议检查数据质量或重新训练模型。")
                proceed_prediction = False
                st.stop()
            
            # ==================== 收入预测部分 ====================
            if proceed_prediction:
                st.markdown("---")
                st.subheader("🎯 FY1收入预测结果")
                
                # 准备特征矩阵（使用已验证的processed_fy1）
                ap_features = get_ap_features()
                paid_features = get_paid_features()
